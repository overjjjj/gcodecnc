...
int main(int argc, char **argv)
{
    ...
    enabledOperation.markToolpathValid();
    ...
    MachiningOperation staleOperation = enabledOperation;
    staleOperation.id = QStringLiteral("stale");
    staleOperation.markToolpathStale(QStringLiteral("setup changed"));
    const QList<MachiningOperation> validExecutable = executableOperations(
        {disabledOperation, staleOperation, enabledOperation});
    if (expect(validExecutable.size() == 1 &&
                   validExecutable.first().id == QStringLiteral("enabled"),
               "final program input must contain only enabled valid operations")) {
        return 1;
    }

    QList<MachiningOperation> editableOperations{enabledOperation};
    StrategyParams editedParams = enabledOperation.params;
    editedParams.set(QStringLiteral("feedRate"), 180.0);
    if (expect(updateOperationParameters(
                   &editableOperations, enabledOperation.id, editedParams),
               "confirmed operation parameters should be editable") ||
        expect(editableOperations.first().toolpathState == ToolpathState::Stale &&
                   editableOperations.first().warnings.contains(
                       QStringLiteral("parameters changed")),
               "parameter edits must invalidate the confirmed toolpath") ||
        expect(editableOperations.first().parameterSources.value(
                   QStringLiteral("feedRate")) ==
                   ProcessParameterSource::ManualOverride,
               "edited parameters must be recorded as manual overrides")) {
        return 1;
    }
